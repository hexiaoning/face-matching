...
Windows 11 桌面端监控视频人脸识别。人员库保存“姓名 + 身份证号 + 1～多张照片”，界面支持本地视频、摄像头、RTSP/RTMP 视频流，识别结果会在视频上标注并记录。
...
2. 双击 `install.cmd`。脚本会创建隔离的 `.venv`、安装 GUI 与 CUDA/cuDNN 运行库，然后执行 GPU 硬校验。如果没有合适的 Python，Windows 11 上会先通过 `winget` 安装用户级 Python 3.12。
3. 双击 `run.cmd`。
4. 首次运行会询问是否下载并校验模型。
...
3. 点“开始识别”。绿色为已确认人员，黄色为采集中/陌生人。身份证号在表格中默认脱敏。
4. 在“设置”中调相似度、质量、检测尺寸等参数。改识别模型后需重启，并在人员库中点“重建当前模型特征”。
