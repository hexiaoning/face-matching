...
Windows 11 桌面端目标人物视频检索：给定一张目标照片，在本地监控录像、摄像头或 RTSP/RTMP 视频流中查找此人是否出现。`v3.1` 使用 1:1 多帧轨迹判定，区分“自动确认”和“待复核候选”，并保留视频时间、最佳人脸截图和支持帧数。人员库功能仍可用于保存常用人员照片。
...
视频/摄像头
   ...
   → 位置 + 运动预测 + 外观特征的人脸轨迹关联
   → 目标模板与轨迹帧做 1:1 比对
   → 多个真实视频帧超过阈值才自动确认；弱证据进入待复核列表
   → 输出视频内时间、最佳证据截图、综合分和支持帧数
```
...
构建脚本会先跑测试，下载并校验固定 SHA-256 的模型，然后把 Python、PySide6、OpenCV、CUDA/OpenVINO GPU 运行库和两套模型一并打入 `dist\FaceMatching-v3.1.0-windows-x64.zip`。目标机不需要 Python、CUDA Toolkit、OpenVINO Toolkit 或互联网；只需预装对应显卡驱动。

目标机可直接双击 `FaceMatching-v3.1.0-Setup.exe` 安装；也可解压便携 ZIP 后双击 `安装并启动 Face Matching.cmd`，自动校验完整性、GPU 和模型后启动。两种方式都不会调用 `winget` 或 `pip`。
...
3. 点“开始检索”。绿色表示同一轨迹已有多个支持帧并达到自动确认阈值；橙色表示分数较高但证据不足，需要查看截图人工复核。
4. 检索完成后查看摘要、视频内时间、最佳证据截图、综合分和支持帧数。不要把“待复核”当作自动命中。
5. 在“设置”中分别调目标确认阈值、候选阈值、最少支持帧、质量和检测尺寸。阈值必须用实际摄像头的正负样本校准。
