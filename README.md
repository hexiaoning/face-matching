...
Windows 11 桌面端监控视频人脸识别。`v3.2` 新增“给定一张照片，搜索此人是否经过”的目标检索模式；也保留人员库 1:N 通用识别。界面支持本地视频、摄像头、RTSP/RTMP 视频流，识别结果会直接标注在视频上。
...
构建脚本会先跑测试，下载并校验固定 SHA-256 的模型，然后把 Python、PySide6、OpenCV、CUDA/OpenVINO GPU 运行库和两套模型一并打入 `dist\FaceMatching-v3.2.0-windows-x64.zip`。目标机不需要 Python、CUDA Toolkit、OpenVINO Toolkit 或互联网；只需预装对应显卡驱动。

目标机可直接双击 `FaceMatching-v3.2.0-Setup.exe` 安装；也可解压便携 ZIP 后双击 `安装并启动 Face Matching.cmd`，自动校验完整性、GPU 和模型后启动。两种方式都不会调用 `winget` 或 `pip`。
...
1. 搜索某个人时，在“视频识别”直接选择目标照片。照片即使裁得较紧，程序也会先补足中性画面上下文再检测；原始人脸内容不会被生成式修改。
2. 选择视频，或输入 `0` 打开本机摄像头，或输入 RTSP/RTMP 地址，然后点“开始识别”。同一轨迹达到多帧支持后显示绿色“目标”，较弱但相似的轨迹显示黄色“疑似目标·待复核”。
3. 不选目标照片时，程序按原有人员库模式工作；可在“人员库”录入姓名、身份证号和 1～多张照片。
4. 在“设置”中可分别调整目标确认/疑似阈值、目标支持帧数，以及通用识别阈值。更改识别模型或镜像 TTA 后需重启，并重建人员库特征。
