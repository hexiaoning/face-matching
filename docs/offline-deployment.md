...
`v2.3` 的正式交付物是 PyInstaller `onedir` 便携包。联网构建机负责下载和固定哈希校验；目标机不运行 `pip`、不下载模型，也不要求预装 Python、CUDA Toolkit 或 cuDNN。
...
输出位于 `dist\FaceMatching-v2.3.0-windows-x64.zip`，旁边的 `.sha256` 文件校验整个压缩包；包内 `SHA256SUMS.txt` 校验每个文件。包中包含：
...
目标机只要求 Windows 11 x64、NVIDIA RTX GPU 和兼容的 NVIDIA 显卡驱动。NVIDIA 驱动无法随应用合法、可靠地通用捆绑，应由设备镜像预装；不需要完整 CUDA Toolkit。

1. 用 `.sha256` 核对 ZIP，解压到本机目录。
2. 在 PowerShell 运行 `verify_offline.ps1`。它会自动发现包内 profile，逐模型校验哈希，并分别执行一次真实 CUDA 推理；任何 CPU fallback 都会直接失败。
