...
“给一张照片，判断此人是否经过”不应直接复用多人图库的开集识别逻辑。v3.4 的目标检索链路会：
...
4. 先用轨迹内特征 medoid 排除疑似串人的身份离群帧，再按媒体时间间隔选取证据；连续相邻帧不能重复增加支持数。
5. 所有具有有效特征的轨迹都进入候选池，阈值只产生“自动确认/待复核/低置信度”标签，最终列表严格按综合分倒序截取 Top-K。
6. 每条轨迹保留最佳时刻、起止时间、最佳人脸截图、支持帧、有效帧和质量。本地文件默认不按原始 FPS 等待，尽快扫描完毕。

默认确认阈值 `0.19`、候选阈值 `0.12` 只用于候选分档，不能直接用于 1:N 人员库识别。自动确认默认关闭；部署方必须先用现场正负样本测量误报率，再决定是否启用。长视频会产生大量比较，不能通过一味降低阈值来修复困难样本。
